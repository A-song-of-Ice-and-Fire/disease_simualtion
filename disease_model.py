...
class DiseaseModel(Model):
    ...
    def __init__(
        ...
        end_point_coo = (center[0] - 62 *  math.floor(games_num/2),center[1])
        for i in range(games_num):
            self.builds.append( 
                Build(
                        self.id_counter , 
                        self, 
                        ( end_point_coo[0] + i * 62 , end_point_coo[1] + self.random.randint(-20,20)),
                        width=self.random.randint(5,10),
                        height=self.random.randint(5,10),
                        watch_time=10
                        )
            )                             
        ...
        #创建观赛者
            ...
            for _ , end_distance in zip(range(individual_nums["worker"]),distances_list):
                ...
                agent.gridInit(None,None,end_distance,in_step=80,waiting_step=1)
                self.schedule.add(agent)
                bar()
